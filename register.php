...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    } elseif(!preg_match('/^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,})$/', trim($_POST["email"]))){
    ...
    if(empty(trim($_POST["mobile"]))){ 
    ...
    } elseif(!preg_match('/^[\+0-9\-\(\)\s]*$/', trim($_POST["mobile"]))){
        $email_err = "Mobile can only contain numbers.";
    } else{
        // Prepare a select statement
        ...
        if($stmt = $mysqli->prepare($sql)){ 
            ...
            if($stmt->execute()){
                ...
                if($stmt->num_rows == 1){
                    $mobile_err = "This mobile is already taken.";
                } else{
